...
# code below to test what types of data there are 

# list_check = []
# for crime in logsof2022:
#   if crime["FIELD2"].split('\n')[0] in list_check:
#     continue
#   print(crime["FIELD2"].split('\n')[0])
#   list_check.append(crime["FIELD2"].split('\n')[0])
def time_difference(time1, time2):
    # Define the time format
    if time1 == '321-22\n 1253' or time2 == '321-22\n 1253':
      return 0
    time_format = "%m-%d-%y\n %H%M"
...
for crime in logsof2022:
  ...
  #Generate the hour the crime occured
  
  Time_Since = crime["FIELD3"]
  time = time_difference(first, Time_Since)

  first = Time_Since

  string = crime["FIELD3"].split('\n')[1].strip()[0:2]
  try:
  ...
  string1 = crime["FIELD4"].split('\n')[0].strip()[0:2]
  try:
    # Try to convert a string to an integer
    number1 = int(string1) -1
  except ValueError:
    # Handle the exception if the conversion fails
    try:
      number1 = int(string1[0:1]) -1
    except ValueError:
      
      continue
     

  #Put the crime into the correct list with the right hour, months 
  
  check = crime["FIELD6"]
  if check == "AN":
    HourListAN[number] += 1
    MonthListAN[number1] += 1
    Time_sinceAN.append(time)
  elif check == "AL":
    HourListAL[number] += 1
    MonthListAL[number1] += 1
    Time_sinceAL.append(time)
  elif check == "LO":
    HourListLO[number] += 1
    MonthListLO[number1] += 1
    Time_sinceLO.append(time)
  elif check == "Fairfax":
    HourListFA[number] += 1
    MonthListFA[number1] += 1
    Time_sinceFA.append(time)
  elif check == "WO":
    HourListWO[number] += 1
    MonthListWO[number1] += 1
    Time_sinceWO.append(time)
  elif check == "MEC":
    HourListME[number] += 1
    MonthListME[number1] += 1
    Time_sinceME.append(time)
  elif check == "MA":
    HourListMA[number] += 1
    MonthListMA[number1] += 1
    Time_sinceMA.append(time)
  elif check == "Reston":
    HourListRE[number] += 1
    MonthListRE[number1] += 1
    Time_sinceRE.append(time)
  elif check == "Bookstore":
    HourListBO[number] += 1
    MonthListBO[number1] += 1
    Time_sinceBO.append(time)

  

#Graph list index 0:8
Graph_List = [HourListAN,
    HourListAL,
    HourListMA,
    HourListWO,
    HourListLO,
...
def plot(plot_id):
    # create the hours vs campus visualization first
    fir = plt.figure(figsize=(5,5))
    plt.xlabel("Hours of the Day")
    plt.ylabel("No. of Crimes Reported")
    if plot_id == 0:
      plt.title("Crimes in Annandale")
    if plot_id == 1:
      plt.title("Crimes in Alexandria")
    if plot_id == 2:
      plt.title("Crimes in Manassas")
    if plot_id == 3:
      plt.title("Crimes in Woodbridge")
    if plot_id == 4:
      plt.title("Crimes in Loudon")
    if plot_id == 5:
      plt.title("Crimes in Bookstore")
    if plot_id == 6:
      plt.title("Crimes in MEC")
    if plot_id == 7:
      plt.title("Crimes in Fairfax")
    if plot_id == 8:
      plt.title("Crimes in Reston")

    plt.style.use('dark_background')
    x = np.arange(0,24)
    y = Graph_List[plot_id]
    ...
    img.seek(0)
    plot_url1 = urllib.parse.quote(base64.b64encode(img.read()).decode())
    plt.clf()
    ...
    # create the Months by Campus visualization
    fir = plt.figure(figsize=(5,5))
    plt.xlabel("Months of the Year")
    plt.ylabel("No. of Crimes Reported")
    if plot_id == 0:
    ...
    img1.seek(0)
    plot_url2 = urllib.parse.quote(base64.b64encode(img1.read()).decode())
    plt.clf()
    # //////////////////////////////////////
    # /////////////////////////////////////
    # Third output 
    # ////////////////////////////////////
    data = Graph_List3[plot_id]
